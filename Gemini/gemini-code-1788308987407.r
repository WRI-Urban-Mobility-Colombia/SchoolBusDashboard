    ...
    if (!is.null(rutas_sub) && nrow(rutas_sub) > 0) {
      ...
      if (var_color %in% names(rutas_sub)) {
        ...
        if (length(valores_unicos) > 0) {
          # Seleccionar paleta estática según el tipo
...
## 3.1.b Observer para actualizar Polígonos y Rutas en Mapa de Clústeres ----##
  observe({
    ...
    # 1. Actualizar Polígonos de la zona
    ...
    # 2. Dibujar las Rutas filtradas correspondientes a los clústeres
...
output$CL_tabla_veh <- renderDT({
  ...
  tabla_resumen <- rutasSubDataset() %>%
    ...
    rename("Tipo de Vehículo" = SR_Veh_Aj_2) %>% 
    # Modificación: Asignar factor por vehículo, dividir y redondear al siguiente entero
    mutate(
      factor = case_when(
